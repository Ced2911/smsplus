...
static __inline__ void cpu_writemem8(unsigned int address, unsigned int data)
{
    ...
    case 0:
        ...
        if (data & 8)
        {
            ...
            z80_map_fetch(0x8000, 0xBFFF, (uint8 *)(sms.sram + offset));
            z80_map_read(0x8000, 0xBFFF, (uint8 *)(sms.sram + offset));
            z80_map_write(0x8000, 0xBFFF, (uint8 *)(sms.sram + offset));
        }
        else
        {
            offset = ((sms.fcr[3] % cart.pages) << 14);
            // vbt 15/05/2008 : exophase :	 data & cart.pages, and set cart.pages to one less than you are
            z80_map_fetch(0x8000, 0xBFFF, (uint8 *)(cart.rom + offset));
            z80_map_read(0x8000, 0xBFFF, (uint8 *)(cart.rom + offset));
            z80_map_write(0x8000, 0xBFFF, (uint8 *)(dummy_write));
        }
        break;
    case 1:
        z80_map_fetch(0x0000, 0x3FFF, (uint8 *)(cart.rom + offset));
        z80_map_read(0x0000, 0x3FFF, (uint8 *)(cart.rom + offset));
        break;

    case 2:
        z80_map_fetch(0x4000, 0x7FFF, (uint8 *)(cart.rom + offset));
        z80_map_read(0x4000, 0x7FFF, (uint8 *)(cart.rom + offset));
        break;
...
#define Z80_MAP_DIRECT 0 /* Reads/writes are done directly */
    ...
    z80_set_in(cz80_z80_readport16);
    z80_set_out(cz80_z80_writeport16);
