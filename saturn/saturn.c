...
SclNorscl Scl_n_reg;
// SclWinscl	Scl_w_reg;
// VBT passage en satic peut poser probl�me ?
...
static void ss_main(void)
{
	...
	unsigned short palette[8] =
		{
			RGB(164 >> 3, 181 >> 3, 197 >> 3),
			RGB(0, 0, 0),
			RGB(164 >> 3, 181 >> 3, 197 >> 3),
			RGB(214 >> 3, 230 >> 3, 247 >> 3),
			RGB(164 >> 3, 181 >> 3, 197 >> 3),
			RGB(0, 0, 0),
			RGB(164 >> 3, 181 >> 3, 197 >> 3),
			RGB(214 >> 3, 230 >> 3, 247 >> 3),
...
//							pcm_PauseOn(pcm);
	...
	if ((device = PER_GetDeviceR(&__port[1], 0)) != NULL)
	{
		...
		if ((pltrigger[1] & PER_DGT_U) != 0)
			temp &= ~0x40;
		if ((pltrigger[1] & PER_DGT_D) != 0)
			temp &= ~0x80;
	}
	return temp;
}
...
static void load_rom(void)
{
	...
	// VBT 11/11/2011 : � remettre si pas assez de m�moire
	if (fileSize < 0x80000)
...
static void displayMenu(void)
{
	...
	scfg.patnamecontrl = 0x000c; // VRAM B1 �̃I�t�Z�b�g
...
void SCL_SetConfig(Uint16 sclnum, SclConfig *scfg)
{
	...
	else
	{
		...
		mapoffsetW = (0x01c0 & ((scfg->plate_addr[0] - SCL_VDP2_VRAM) / boundary)) >> 6;
		*mapoffset |= mapoffsetW << shift;
	}

	for (i = 0; i < max; i++)
	{
		map[i] = (0x003f & ((scfg->plate_addr[i * 2] - SCL_VDP2_VRAM) / boundary));
		temp = (0x003f & ((scfg->plate_addr[i * 2 + 1] - SCL_VDP2_VRAM) / boundary)) << 8;
...
static void drawWindow(unsigned int l1, unsigned int l2, unsigned int l3, unsigned int vertleft, unsigned int vertright)
{
	...
	for (x = 0; x < l2; x++)
	{
		for (j = 0; j < vertleft; j++)
			*VRAM++ = 0x2222; // barre verticale gauche
		for (j = 0; j < 128 - vertleft - vertright; j++)
			*VRAM++ = 0x0000; // noir
		for (j = 0; j < vertright; j++)
			*VRAM++ = 0x2222; // barre verticale droite
	}
